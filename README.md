...
**Swagger UI** → https://goodsorder.n-e.kr/swagger-ui/index.html
...
기존 **Java JDBC 기반으로 개발된 쇼핑몰 애플리케이션**을 현대적인 백엔드 아키텍처로 개선하기 위해 **Spring Boot 4.0 기반의 RESTful API 서버**로 리팩토링한 프로젝트입니다.

**Spring Data JPA**를 적용하여 데이터 접근 계층의 생산성과 유지보수성을 향상시켰으며, **Spring Security 7과 JWT 기반 인증·인가 체계**를 구축하여 보안성을 강화하였습니다. **비관적 락(Pessimistic Lock)**을 활용한 재고 동시성 제어, **전역 예외 처리**, **트랜잭션 관리** 등 실무에서 반드시 고려해야 할 핵심 설계 요소들을 직접 구현하였습니다.

**Docker 컨테이너 환경**에서 **AWS EC2**에 배포하고, **GitHub Actions**를 활용한 CI/CD 파이프라인을 구성하여 안정적인 자동 배포 환경을 구현하였습니다. **Nginx** 리버스 프록시와 **Let's Encrypt SSL** 인증서를 적용하여 HTTPS 기반의 운영 환경을 구축하였습니다.

아울러 **Springdoc OpenAPI(Swagger UI)**를 통한 API 문서화와 **JUnit 5, Mockito** 기반 서비스 레이어 단위 테스트를 작성하여 개발 생산성과 서비스 품질을 높였으며, 실무 환경에 가까운 백엔드 개발 및 운영 경험을 목표로 프로젝트를 진행하였습니다.
